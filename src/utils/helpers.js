export async function accountData(id) {  
    ...
    const insights = await fetch("http://localhost:3000/api/insights", {
        headers: {
            "Content-Type": "application/json",
        },
        method: "POST",
        body: JSON.stringify({ 
          accountId:  result.data.records[0].id,
        }),
        cache: 'no-store',
      },
    ); 
...
export const setAccount360Dashboard = async (sfdcAccountId) => {
    const embed = await miro.board.createEmbed({
       ...
       height: 540.43535371989,
       width: 1227.665742820446,
       x: -337.3496481444108,
       y: -13813.376796592242
    })
